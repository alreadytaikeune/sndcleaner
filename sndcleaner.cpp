...
void SndCleaner::packet_queue_init(PacketQueue *q) {
...
/*
* Check if there is enough data to fill the stream buffer to be processed
*/
bool SndCleaner::is_queue_full(){
	if(! &audioq){
...
bool SndCleaner::is_queue_empty(){
	if(! &audioq){
		std::cerr << "Packets queue is null" << std::endl;
		exit(EXIT_FAILURE);
	}
	return audioq.last_pkt;
}
